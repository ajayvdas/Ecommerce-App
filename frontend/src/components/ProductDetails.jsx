...
function ProductDetails({ product }) {
  ...
  return (
    <motion.div
      ...
      className="space-y-6 lg:space-y-6 xl:space-y-7"
    >
      {/* Category Badge */}
      ...
      {/* Product Title */}
      <h1 className="text-3xl lg:text-4xl xl:text-3xl font-bold text-gray-900 leading-tight">{product.name}</h1>
      ...
      {/* Price */}
      <div className="text-3xl lg:text-4xl xl:text-3xl font-bold text-gray-900">${product.price.toFixed(2)}</div>

      {/* Delivery Information */}
      <div className="flex items-center space-x-2 text-sm lg:text-base text-gray-600">
        <Clock className="w-4 h-4 lg:w-5 lg:h-5" />
        <span>
          Order in <span className="font-semibold text-gray-900">{deliveryTime}</span> to get next day delivery
        </span>
      </div>
